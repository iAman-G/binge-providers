(function(global2, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("iso-639-1"), require("nanoid"), require("unpacker"), require("crypto-js"), require("cheerio"), require("form-data"), require("cookie"), require("set-cookie-parser"), require("hls-parser")) : typeof define === "function" && define.amd ? define(["exports", "iso-639-1", "nanoid", "unpacker", "crypto-js", "cheerio", "form-data", "cookie", "set-cookie-parser", "hls-parser"], factory) : (global2 = typeof globalThis !== "undefined" ? globalThis : global2 || self, factory(global2.index = {}, global2["iso-639-1"], global2.nanoid, global2.unpacker, global2["crypto-js"], global2.cheerio, global2["form-data"], global2.cookie, global2["set-cookie-parser"], global2["hls-parser"]));
})(this, function(exports2, ISO6391, nanoid$1, unpacker, CryptoJS, cheerio, FormData, cookie, setCookieParser, hlsParser) {
  "use strict";
  ...
  function getTargetFeatures(target, consistentIpForRequests, proxyStreams) {
    ...
    if (proxyStreams)
      features.disallowed.push(flags.PROXY_BLOCKED);
    return features;
  }
  ...
  const droploadScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const unpacked = unpacker.unpack(evalCode[1]);
  ...
  const randomId = nanoid$1.customAlphabet("1234567890abcdef");
  ...
  async function getSubtitles(ctx, id, fid, type, episodeId, seasonId) {
    const module2 = type === "movie" ? "Movie_srt_list_v2" : "TV_srt_list_v2";
  ...
  const turbovidScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      if (!apkey || !xxid)
        throw new Error("Failed to get required values");
  ...
  const vTubeScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const mainPage = mainPageRes.body;
      const html = cheerio.load(mainPage);
      const evalCode = html("script").text().match(evalCodeRegex$2);
      if (!evalCode)
        throw new Error("Failed to find eval code");
      const unpacked = unpacker.unpack(evalCode == null ? void 0 : evalCode.toString());
  ...
  async function comboScraper$5(ctx) {
    ...
    const watchPage = cheerio.load(await ctx.proxiedFetcher(watchPageUrl));
    ctx.progress(80);
  ...
  async function getSource(ctx, sources, title) {
    const source = cheerio.load(sources)(`a[title*=${title} i]`);
  ...
  const bflixScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      if (!evalCode)
        throw new Error("Failed to find eval code");
      const unpacked = unpacker.unpack(evalCode[0]);
      const file = unpacked.match(mp4Regex);
      if (!(file == null ? void 0 : file[0]))
        throw new Error("Failed to find file");
      return {
        stream: [
          {
  ...
  const fileMoonScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      const embedHtml = cheerio.load(embedRes);
  ...
  const fileMoonMp4Scraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      ...
      if (!result.stream)
        throw new NotFoundError("Failed to find result");
      if (result.stream[0].type !== "hls")
        throw new NotFoundError("Failed to find hls stream");
  ...
  const hydraxScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      let qualityData = {};
      if (qualityMatch == null ? void 0 : qualityMatch[1])
        qualityData = JSON.parse(qualityMatch[1]);
      const data2 = JSON.parse(atob(match[1]));
      if (!data2.id || !data2.domain)
  ...
  const playm4uNMScraper = makeEmbed({
    ...
    scrape: async (ctx) => {
      var _a, _b;
      const mainPage$ = cheerio.load(await ctx.proxiedFetcher(ctx.url));
  ...
  const wootlyScraper = makeEmbed({
    ...
    async scrape(ctx) {
      ...
      let $ = cheerio.load(wootlyData.body);
  ...
  const universalScraper$5 = async (ctx) => {
    ...
    ctx.progress(50);
    let watchPage$ = cheerio.load(watchPage.body);
    ...
    if (!(laravelSession == null ? void 0 : laravelSession.value))
  ...
  const universalScraper$4 = async (ctx) => {
    ...
    const videoPage$ = cheerio.load(videoPage);
  ...
  async function comboScraper(ctx) {
    ...
    if (!embedUrl)
      throw new Error("Failed to find embed url");
    const embedPage = cheerio.load(await ctx.proxiedFetcher(embedUrl));
  ...
  async function getStreams(title) {
    const titlePage = cheerio.load(title);
  ...
  const primewireScraper = makeSourcerer({
    ...
    async scrapeShow(ctx) {
      ...
      const seasonPage = cheerio.load(season);
  ...
  const universalScraper$1 = async (ctx) => {
    ...
    const searchResult$ = cheerio.load(searchResult);
  ...
  const warezcdnScraper = makeSourcerer({
    ...
    scrapeMovie: async (ctx) => {
      ...
      const $ = cheerio.load(serversPage);
